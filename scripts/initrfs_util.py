...
def create(file_name, dir_name):
    ...
    def create_file(name, path):
        ...
        fd.close()
        out_fd.write(struct.pack("L", len(file_content))) # file size
...
def main():
    if len(sys.argv) <= 1:
        ...
        print("Commands:")
        print("  create <file> <directory> | create an initial ram disk image file from a directory")
        print("  extract <file> <directory> | extract the initial ram disk image file contents into a directory")
